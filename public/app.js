...
const E2E = {
  ...
  /* Выведение общего AES-GCM ключа */
  ...
  async encrypt(sharedKey, plainText) {
    ...
    // Упаковываем iv + шифртекст → base64
    const result = new Uint8Array(iv.byteLength + enc.byteLength);
    result.set(iv, 0);
    result.set(new Uint8Array(enc), iv.byteLength);
  ...
  /* Хэширование пароля — PBKDF2 + SHA-256 */
