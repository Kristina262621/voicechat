// ═══════════════════════════════════════════════
//  APP.JS — главный файл
//  Зависит от: core.js, ui.js, status.js, invitelink.js
// ═══════════════════════════════════════════════
...
//  DOM — ждём загрузки
// ═══════════════════════════════════════════════
// Все DOM-переменные объявляем после загрузки
// чтобы избежать null при раннем выполнении
...
function openPeerProfile(nickname, avatar) {
  ...
  socket.emit('profile-get-user', { nickname }, res => {
    ...
    if (statusEl) {
      if (res.online) {
        statusEl.innerHTML = '<span style="color:#3dba6e">● в сети</span>';
      } else {
  ...
  sheet.querySelector('#peer-action-notif')?.addEventListener('click', () => {
    toggleMenu();
    if (typeof openChatNotifSettings === 'function') openChatNotifSettings(currentChatId || nickname, nickname);
  });
  ...
  sheet.querySelector('#peer-call-btn')?.addEventListener('click', () => {
    close();
    setTimeout(() => { openPrivateChatWith(nickname); setTimeout(() => startPrivateCall(false), 800); }, 400);
  });
  sheet.querySelector('#peer-msg-btn')?.addEventListener('click', () => {
    close(); setTimeout(() => openPrivateChatWith(nickname), 400);
  });
...
function doLogin() {
  ...
  socket.emit('auth-login', { nickname: nick, password: pw }, res => {
    ...
    if (res.ok) {
      ...
      if (loginPw) { loginPw.style.animation = 'shake 0.35s'; setTimeout(() => { loginPw.style.animation = ''; }, 400); }
    }
  });
}
...
function openNewChatMenu() {
  ...
  inner.addEventListener('touchend',   e => { const dy=e.changedTouches[0].clientY-startY; inner.style.transform='';inner.style.opacity=''; if(dy>80)close(); }, { passive: true });
...
function openUserSearchForChat() {
  ...
  const doSearch = () => {
    const nick = input.value.trim().replace(/^@/, ''); if (!nick) return;
...
function renderUnifiedList() {
  ...
  let html = '';
  if (groups.length)   { html += `<div class="chat-list-section-title">👥 Группы</div>`;     html += groups.map(buildRoomCardHTML).join(''); }
  if (privates.length) { html += `<div class="chat-list-section-title">💬 Личные чаты</div>`; html += privates.map(buildPrivateCardHTML).join(''); }
...
function renderUnifiedListInChat() {
  ...
  let html = '';
  if (groups.length)   { html += `<div class="chat-list-section-title">👥 Группы</div>`;  html += groups.map(buildRoomCardSmallHTML).join(''); }
  if (privates.length) { html += `<div class="chat-list-section-title">💬 Личные</div>`; html += privates.map(buildPrivateCardSmallHTML).join(''); }
...
async function enterPrivateChat(chatId, withNickname, withAvatar) {
  ...
  // Загружаем реальный статус
  if (typeof loadInitialStatus === 'function') {
    loadInitialStatus(withNickname);
  } else {
      ...
      for (const msg of res.messages) {
        ...
        if (msg.type==='voice') {
          ...
          try {
            ...
            const domId=appendMessage({id:msg.id,nickname:msg.fromNick,text,type:'text',timestamp:msg.timestamp,mine,status:'ok',msgStatus:mine?(msg.status||'sent'):null,edited:msg.edited,replyTo:msg.replyTo||null});
            if(msg.id&&mine)msgIdToDomId.set(msg.id,domId);
...
socket.on('private-typing-start',({chatId,fromNick})=>{
  ...
  window._privateTypingClearTimer=setTimeout(()=>{
    // После окончания печати — восстанавливаем реальный статус
...
socket.on('private-message', async data => {
  ...
  const domId = appendMessage({id:data.id,nickname:data.fromNick,type:data.type,fileName:data.fileName,fileSize:data.fileSize,mimeType:data.mimeType,timestamp:data.timestamp,mine:false,status:'decrypting',replyTo:data.replyTo||null});
  try {
    if (data.type==='text') {
...
function openMsgContextMenu(domId, msgEl) {
  ...
  // Реакции и ответ добавляем через requestAnimationFrame
  requestAnimationFrame(() => {
    ...
    replyBtn.addEventListener('click', () => { menu.remove(); setReplyTo(msgIdAttr, msgEl); });
        ...
        if (histRes.ok && histRes.messages && histRes.messages.length) {
          for (const msg of histRes.messages) {
            ...
            if (msg.type === 'voice') {
              ...
              try {
                const text   = await Crypto.decryptText(msg.encrypted, msg.iv);
                const domId  = appendMessage({id:msg.id,nickname:msg.nickname,text,type:'text',timestamp:msg.timestamp,mine,status:'ok',edited:msg.edited,replyTo:msg.replyTo||null});
                if (msg.id && mine) msgIdToDomId.set(msg.id, domId);
...
function openMembersModal() {
  ...
  // Кнопка "Выйти из группы" для не-владельцев
  ...
  if (!isRoomOwner) {
    setTimeout(() => {
      ...
      wrap.innerHTML = `
        <button id="btn-leave-group-modal" class="btn-danger">
          🚪 Выйти из группы
        </button>`;
      ...
      document.getElementById('btn-leave-group-modal')?.addEventListener('click', () => {
        ...
        socket.emit('leave-room-permanent', {roomId: currentRoomId}, res => {
          if (res.ok) {
            ...
            leaveCurrentRoom(); showScreen('lobby'); showToast('✅ Вы вышли из группы');
          } else {
            const msgs = { owner_cannot_leave: '❌ Владелец не может выйти. Удалите группу.', not_found: '❌ Группа не найдена' };
  ...
  // Ссылка-приглашение
  setTimeout(() => {
    if (typeof addInviteLinkButton === 'function') addInviteLinkButton();
  }, 200);
}
...
function renderTyping() {
  const el = getHeaderSubEl(); if (!el || currentChatType === 'private') return;
...
async function sendTextMessage() {
  ...
  try {
    if (currentChatType === 'private' && currentChatId) {
      ...
      const domId = appendMessage({nickname:myNickname, text, type:'text', timestamp:Date.now(), mine:true, status:'ok', msgStatus:'sending', replyTo:currentReply});
      ...
      const {encrypted, iv} = await Crypto.encrypt(text);
      const seq = ++outgoingSeq;
      const domId = appendMessage({from:socket.id, nickname:myNickname, text, type:'text', timestamp:Date.now(), mine:true, status:'ok', msgStatus:'sending', replyTo:currentReply});
...
async function sendVoiceMessage(blob, duration, mimeType) {
  try {
    ...
    const domId    = appendMessage({from:socket.id, nickname:myNickname, type:'voice', localUrl, duration, mimeType, timestamp:Date.now(), mine:true, status:'ok', msgStatus:'sending'});
    msgIdToDomId.set('pending-' + seq, domId);
    if (currentChatType === 'private' && currentChatId) {
...
async function sendMediaBlob(blob, mimeType, fileName, type) {
  ...
  try {
    const ab = await blob.arrayBuffer();
    let fakeProgress = 0;
    const progressInterval = setInterval(() => { fakeProgress = Math.min(fakeProgress + 5, 90); updateUploadProgress(fakeProgress); }, 100);
    const {encrypted, iv} = await Crypto.encrypt(ab);
    clearInterval(progressInterval); updateUploadProgress(95);
    const localUrl = URL.createObjectURL(new Blob([ab], {type: mimeType}));
    const seq      = ++outgoingSeq;
    const payload  = {encrypted, iv, type, seq, fileName: fileName||'file', fileSize: blob.size, mimeType};
    const domId    = appendMessage({from:socket.id, nickname:myNickname, type, localUrl, fileName:fileName||'file', fileSize:blob.size, mimeType, timestamp:Date.now(), mine:true, status:'ok', msgStatus:'sending'});
    msgIdToDomId.set('pending-' + seq, domId);
    if (currentChatType === 'private' && currentChatId) {
      socket.emit('private-message', {chatId:currentChatId, ...payload}, res => {
  ...
  } catch (e) { hideUploadProgress(); hideChatUploadStatus(); showToast('❌ Ошибка отправки: ' + e.message); }
}
...
socket.on('chat-message', async data => {
  ...
  if (data.type === 'voice') {
    ...
    appendMessage({id:data.id, from:data.from, nickname:data.nickname, type:'voice', duration:data.duration||0, timestamp:data.timestamp, mine:false, status:'ok', encrypted:data.encrypted, iv:data.iv, mimeType:data.mimeType});
    if (document.visibilityState !== 'visible') addUnread(chatId, 1);
    return;
  }
  const domId = appendMessage({id:data.id, from:data.from, nickname:data.nickname, type:data.type, fileName:data.fileName, fileSize:data.fileSize, mimeType:data.mimeType, timestamp:data.timestamp, mine:false, status:'decrypting', replyTo:data.replyTo||null});
  try {
    if (data.type === 'text') {
      const text = await Crypto.decryptText(data.encrypted, data.iv);
      updateMessage(domId, {text, status:'ok'});
      if (setting !== 'none') { if (setting !== 'mute') playMsgSound(chatId); showBrowserNotif('💬 ' + (data.nickname||'?'), text, chatId); }
      if (document.visibilityState !== 'visible') addUnread(chatId, 1);
    } else {
      const mime = data.mimeType || 'application/octet-stream';
      const blob = await Crypto.decryptBlob(data.encrypted, data.iv, mime);
...
function updateMessage(id, updates) {
  ...
  if (content) {
    const merged = {type:div.dataset.type, mimeType:div.dataset.mimeType, fileName:div.dataset.fileName, fileSize:div.dataset.fileSize, duration:div.dataset.duration, ...updates};
...
function buildVoiceMessageHTML(msg) {
  ...
  const bars   = Array.from({length: 20}, () => { const h=Math.floor(Math.random()*16+4); return `<div class="voice-msg-bar" style="height:${h}px"></div>`; }).join('');
  if (msg.localUrl) {
    return `<div class="voice-msg" id="${vmId}" data-dur="${dur}"><button class="voice-msg-btn" data-url="${msg.localUrl}">▶️</button><div class="voice-msg-waveform">${bars}</div><span class="voice-msg-duration">${durStr}</span></div>`;
  }
  return `<div class="voice-msg" id="${vmId}" data-encrypted="${msg.encrypted||''}" data-iv="${msg.iv||''}" data-mime="${msg.mimeType||'audio/webm'}" data-dur="${dur}"><button class="voice-msg-btn voice-decrypt-btn">▶️</button><div class="voice-msg-waveform">${bars}</div><span class="voice-msg-duration">${durStr}</span></div>`;
}

function playVoiceMsg(btn, url, wrap) {
  if (!wrap) wrap = btn.closest('.voice-msg');
  if (currentVoiceAudio && !currentVoiceAudio.paused) {
    currentVoiceAudio.pause(); currentVoiceAudio.currentTime = 0;
  ...
  currentVoiceAudio = audio; currentVoiceBtn = btn; btn.textContent = '⏸️';
  audio.play().then(() => {
    ...
    audio.onended = () => {
      btn.textContent = '▶️'; bars.forEach(b => b.classList.remove('active'));
...
function bindMediaEvents(container) {
  ...
  container.querySelectorAll('.voice-msg-btn[data-url]').forEach(btn => {
    btn.addEventListener('click', () => { const wrap = btn.closest('.voice-msg'); playVoiceMsg(btn, btn.dataset.url, wrap); });
  });
  container.querySelectorAll('.voice-decrypt-btn').forEach(btn => {
    btn.addEventListener('click', async function() {
      ...
      try {
        ...
        btn.classList.remove('voice-decrypt-btn'); btn.dataset.url = url; btn.textContent = '▶️';
  ...
  // Клик по блоку reply → прокрутка
  container.querySelectorAll('.msg-reply-block').forEach(block => {
    block.addEventListener('click', () => {
      const replyId = block.dataset.replyId; if (!replyId) return;
      let target = document.querySelector(`[data-msg-id="${replyId}"]`);
      if (!target) { const domId = msgIdToDomId.get(replyId); if (domId) target = document.getElementById(domId); }
...
//  РЕАКЦИИ
// ═══════════════════════════════════════════════
function getClientReactions(msgId) { return clientReactions.get(msgId) || {}; }
...
function buildReactionsHTML(msgId, reactions) {
  ...
  const myLower = myNickname.toLowerCase();
  const html = Object.entries(reactions).filter(([,users])=>users.length>0).map(([emoji,users])=>{
...
function openReactionPicker(msgId, msgEl) {
  ...
  popup.innerHTML = REACTION_EMOJIS.map(emoji => `<button class="rp-btn" data-emoji="${emoji}" style="width:36px;height:36px;border:none;background:none;font-size:22px;cursor:pointer;border-radius:10px;">${emoji}</button>`).join('');
  document.body.appendChild(popup);
  popup.querySelectorAll('.rp-btn').forEach(btn => { btn.addEventListener('click', () => { popup.remove(); toggleReaction(msgId, btn.dataset.emoji, msgEl); }); });
...
function toggleReaction(msgId, emoji, msgEl) {
  ...
  if (iMine) socket.emit('remove-reaction', {msgId, emoji, chatId, roomId}, res => { if (res.ok) updateMsgReactions(msgId, res.reactions, msgEl); });
  else       socket.emit('add-reaction',    {msgId, emoji, chatId, roomId}, res => { if (res.ok) updateMsgReactions(msgId, res.reactions, msgEl); });
}
...
function updateMsgReactions(msgId, reactions, msgEl) {
  ...
  if (!msgEl) {
    ...
    if (!msgEl) { for (const [,domId] of msgIdToDomId) { const el=document.getElementById(domId); if(el&&el.dataset.msgId===msgId){msgEl=el;break;} } }
  }
  if (!msgEl) return;
  const reactionsEl = msgEl.querySelector('.msg-reactions');
  const newHTML     = buildReactionsHTML(msgId, reactions);
  if (reactionsEl) { if (newHTML) reactionsEl.outerHTML = newHTML; else reactionsEl.remove(); }
  else if (newHTML) { const meta=msgEl.querySelector('.msg-meta'); if(meta)meta.insertAdjacentHTML('beforebegin',newHTML); else msgEl.insertAdjacentHTML('beforeend',newHTML); }
}

socket.on('reaction-updated', ({msgId, reactions}) => {
  setClientReactions(msgId, reactions);
  let msgEl = null;
  for (const [mId, domId] of msgIdToDomId) { if (mId === msgId) { msgEl = document.getElementById(domId); break; } }
  if (!msgEl) document.querySelectorAll('.msg').forEach(el => { if (el.dataset.msgId === msgId) msgEl = el; });
...
document.addEventListener('click', e => {
  ...
  const msgId = btn.dataset.msgid; const emoji = btn.dataset.emoji;
...
function setReplyTo(msgId, msgEl) {
  ...
  const nickname = msgEl?.querySelector('.msg-sender')?.textContent?.replace('👤 ','').trim() || (msgEl?.classList.contains('mine') ? myNickname : '?');
  replyToMsg = {id: msgId, nickname, text};
  let bar = document.getElementById('reply-bar');
  if (!bar) {
    bar = document.createElement('div'); bar.id = 'reply-bar';
...
function cancelReply() {
  replyToMsg = null;
  const bar = document.getElementById('reply-bar'); if (bar) bar.remove();
  if (window._updateChatLayout) window._updateChatLayout();
}
    ...
    timer = setTimeout(() => {
      socket.emit('search-chats', {query: q}, res => {
        ...
        if (res.rooms && res.rooms.length) {
          ...
          html += res.rooms.map(r=>`<div class="room-card search-result-room" data-id="${r.id}" data-has-pw="${r.hasPassword||false}" data-joinmode="${r.joinMode||'open'}" data-name="${escapeHtml(r.name)}" style="cursor:pointer;"><div class="room-avatar">${r.photo?`<img src="${escapeHtml(r.photo)}" alt="">`:'🏠'}</div><div class="room-info"><div class="room-name">${escapeHtml(r.name)}</div><div class="room-meta">👥 ${r.memberCount} участников</div></div></div>`).join('');
        }
        if (res.users && res.users.length) {
          html += '<div class="chat-list-section-title">👤 Пользователи</div>';
          html += res.users.map(u=>`<div class="pc-card search-result-user" data-nick="${escapeHtml(u.nickname)}" style="cursor:pointer;"><div class="room-avatar">👤</div><div class="room-info"><div class="room-name">${escapeHtml(u.nickname)}</div><div class="room-meta">@${escapeHtml(u.lower)}</div></div></div>`).join('');
        }
        if (!html) html = '<div style="text-align:center;color:var(--sub);font-size:14px;padding:40px 20px">Ничего не найдено</div>';
        results.innerHTML = html;
        results.querySelectorAll('.search-result-room').forEach(card => { card.addEventListener('click', () => { close(); joinRoom(card.dataset.id, ''); }); });
        results.querySelectorAll('.search-result-user').forEach(card => { card.addEventListener('click', () => { close(); openPrivateChatWith(card.dataset.nick); }); });
      });
    }, 300);
  });
}
...
function openEmojiPicker() {
  ...
  const popup     = document.createElement('div'); popup.id = 'emoji-picker-popup';
  const tgBottom  = document.getElementById('tg-bottom');
  ...
  popup.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:4px;max-height:220px;overflow-y:auto;">${EMOJI_LIST.map(emoji=>`<button class="ep-btn" style="width:36px;height:36px;border:none;background:none;font-size:22px;cursor:pointer;border-radius:10px;">${emoji}</button>`).join('')}</div>`;
  document.body.appendChild(popup);
  ...
  setTimeout(() => { document.addEventListener('click', function h(e) { if(!popup.contains(e.target)&&e.target.id!=='btn-emoji-picker'){popup.remove();document.removeEventListener('click',h);} }); }, 50);
}
...
function renderSignal(userId, level) {
  const w = document.getElementById('sig-' + userId); if (!w) return;
...
async function measureQuality(peer, isLocal) {
  try {
    ...
    stats.forEach(r => {
      if (r.type === 'candidate-pair' && r.state === 'succeeded') { if (r.currentRoundTripTime != null) rtt = r.currentRoundTripTime * 1000; }
      if (isLocal) {
        if (r.type === 'remote-inbound-rtp' && r.kind === 'audio') {
          if (r.roundTripTime != null) rtt = r.roundTripTime * 1000;
          if (r.jitter != null) jitter = r.jitter;
          if (r.packetsLost != null) { stats.forEach(s => { if (s.type==='outbound-rtp'&&s.kind==='audio'&&s.packetsSent>0) lostRatio=Math.max(0,r.packetsLost)/(Math.max(0,r.packetsLost)+s.packetsSent); }); }
        }
      } else {
...
function startQualityMonitor(userId, peer, isLocal) {
  ...
  qualityTimers[userId] = setInterval(async () => { const level = await measureQuality(peer, isLocal); renderSignal(userId, level); }, 3000);
}
...
function addParticipant(userId, nickname, isMe) {
  ...
  div.innerHTML = `<span class="participant-name" id="pname-${userId}">${displayName}</span><div class="volume-bar-wrap"><div class="volume-bar" id="vol-${userId}"></div></div><div class="signal-wrap signal-none" id="sig-${userId}"><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div></div>${understoodBtn}`;
  participantsList.appendChild(div);
  div.querySelector('.btn-understood')?.addEventListener('click', function() {
    socket.emit('understood'); this.textContent = '✅'; this.disabled = true;
...
function removeParticipant(userId) {
  ...
  if (participantsList && !participantsList.children.length && participantsBox) participantsBox.style.display = 'none';
}
...
function startVolumeAnalysis(userId, stream) {
  ...
  } catch (e) { console.warn('startVolumeAnalysis error:', e); }
}
...
async function getMicStream() {
  try {
    return await navigator.mediaDevices.getUserMedia({video:false,audio:{echoCancellation:{ideal:true},noiseSuppression:{ideal:true},autoGainControl:{ideal:true},sampleRate:{ideal:48000},channelCount:{ideal:1}}});
  } catch (e) {
    return await navigator.mediaDevices.getUserMedia({video:false,audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});
  }
}
...
async function handleOffer(from, offer, nickname) {
  ...
  const peer = new RTCPeerConnection(iceServers); peers[from] = peer;
  ...
  peer.ontrack = e => {
    ...
    if (!audio) { audio=document.createElement('audio'); audio.id='audio-'+from; audio.autoplay=true; audio.playsInline=true; if(hiddenAudios)hiddenAudios.appendChild(audio); }
    if (audio.srcObject !== st) { audio.srcObject=st; audio.play().then(()=>startVolumeAnalysis(from,st)).catch(()=>{document.addEventListener('click',()=>audio.play().catch(()=>{}),{once:true});}); }
  };
  peer.onicecandidate = e => { if (e.candidate) socket.emit('ice-candidate', {to:from, candidate:e.candidate}); };
  peer.oniceconnectionstatechange = () => { const s=peer.iceConnectionState; if(s==='failed')peer.restartIce(); if(s==='disconnected')setTimeout(()=>{if(peer.iceConnectionState==='disconnected')peer.restartIce();},3000); };
  peer.onconnectionstatechange = () => { if (peer.connectionState==='connected') startQualityMonitor(from, peer, false); };
  try {
...
function createPeer(userId, isInitiator) {
  ...
  peer.getSenders().forEach(s => {
    if (s.track?.kind === 'audio') { const p=s.getParameters(); if(!p.encodings)p.encodings=[{}]; p.encodings[0].maxBitrate=40000; p.encodings[0].priority='high'; s.setParameters(p).catch(()=>{}); }
  });
  let restartAttempts = 0, restartTimer = null;
  function tryRestart() {
    if (restartAttempts >= 5) return; restartAttempts++;
    clearTimeout(restartTimer);
    const delay = Math.min(1500 * Math.pow(2, restartAttempts-1), 20000);
    restartTimer = setTimeout(() => { if(peer.connectionState==='failed'||peer.iceConnectionState==='failed')peer.restartIce(); }, delay);
  }
  peer.addEventListener('connectionstatechange', () => {
    const state = peer.connectionState;
    if (state==='connected') { restartAttempts=0;clearTimeout(restartTimer);startQualityMonitor(userId,peer,isInitiator);if(isInitiator&&!qualityTimers[socket.id])startQualityMonitor(socket.id,peer,true); }
    if (state==='failed') tryRestart();
    if (state==='disconnected') { restartTimer=setTimeout(()=>{if(peer.connectionState==='disconnected'||peer.connectionState==='failed')tryRestart();},3000); }
  });
  peer.ontrack = e => {
    const trackStream = e.streams&&e.streams[0]?e.streams[0]:new MediaStream([e.track]);
    if (e.track.kind !== 'audio') return;
    let audio = document.getElementById('audio-' + userId);
    if (!audio) { audio=document.createElement('audio'); audio.id='audio-'+userId; audio.autoplay=true; audio.playsInline=true; if(hiddenAudios)hiddenAudios.appendChild(audio); }
    if (audio.srcObject !== trackStream) { audio.srcObject=trackStream; audio.play().then(()=>startVolumeAnalysis(userId,trackStream)).catch(()=>{document.addEventListener('click',()=>audio.play().catch(()=>{}),{once:true});}); }
  };
  peer.onicecandidate = e => { if (e.candidate) socket.emit('ice-candidate', {to:userId, candidate:e.candidate}); };
  peer.oniceconnectionstatechange = () => { const s=peer.iceConnectionState; if(s==='failed')tryRestart(); if(s==='disconnected')setTimeout(()=>{if(peer.iceConnectionState==='disconnected')tryRestart();},3000); };
  if (isInitiator) {
    let offerSent = false;
    peer.onnegotiationneeded = async () => {
      ...
      } catch (e) { console.error('createOffer error:', e); offerSent = false; }
    };
  }
  return peer;
}
...
function showCallMiniBar(name, avatar) {
  ...
  if (avatarEl) { if(avatar)avatarEl.innerHTML=`<img src="${avatar}" style="width:100%;height:100%;border-radius:50%;object-fit:cover">`;else avatarEl.textContent='👤'; }
  bar.classList.add('visible');
  bar.onclick = e => { if(e.target.id==='call-mini-hangup')return; const img=$('call-screen-avatar')?.querySelector('img')?.src||null; showCallScreen(pcCallRemoteNick,img,null,pcCallIsVideo); };
}
function hideCallMiniBar() { const bar=$('call-mini-bar');if(bar)bar.classList.remove('visible'); }
function updateCallMiniStatus(text) { const el=$('call-mini-status-text');if(el)el.textContent=text||'Звонок…'; }

// ═══════════════════════════════════════════════
//  ЗВОНКИ — ЗВУКИ
// ═══════════════════════════════════════════════
function playIncomingRing() {
  stopIncomingRing(); let count = 0;
  const playOnce = () => {
    if (count++ > 30) { stopIncomingRing(); return; }
    try {
      const ctx=new(window.AudioContext||window.webkitAudioContext)(); ringToneCtx=ctx;
      const gain=ctx.createGain(); gain.connect(ctx.destination);
      [{freq:480,time:0,dur:0.18},{freq:640,time:0.20,dur:0.18},{freq:800,time:0.40,dur:0.28}].forEach(note=>{
        const osc=ctx.createOscillator(); osc.type='sine'; osc.frequency.setValueAtTime(note.freq,ctx.currentTime+note.time); osc.connect(gain);
        gain.gain.setValueAtTime(0,ctx.currentTime+note.time); gain.gain.linearRampToValueAtTime(0.35,ctx.currentTime+note.time+0.02); gain.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+note.time+note.dur);
  ...
  playOnce(); ringInterval = setInterval(playOnce, 2000);
}
function stopIncomingRing() { if(ringInterval){clearInterval(ringInterval);ringInterval=null;} if(ringToneCtx){try{ringToneCtx.close();}catch(_){}ringToneCtx=null;} }

function playDialTone() {
  stopDialTone(); let count = 0;
  const playOnce = () => {
    ...
    try {
      ...
      osc1.type='sine'; osc1.frequency.value=425; osc2.type='sine'; osc2.frequency.value=450;
      gain.gain.setValueAtTime(0.15,ctx.currentTime); gain.gain.setValueAtTime(0.15,ctx.currentTime+1.0); gain.gain.linearRampToValueAtTime(0,ctx.currentTime+1.05);
      osc1.start(ctx.currentTime); osc1.stop(ctx.currentTime+1.1); osc2.start(ctx.currentTime); osc2.stop(ctx.currentTime+1.1);
      setTimeout(()=>{try{ctx.close();}catch(_){}},2500);
    } catch(_) {}
  };
  playOnce(); dialToneInterval = setInterval(playOnce, 4000);
}
function stopDialTone() { if(dialToneInterval){clearInterval(dialToneInterval);dialToneInterval=null;} }
...
function setSpeakerOutput(external) {
  ...
  if (callBtnSpeaker) { if(external){callBtnSpeaker.textContent='🔊';callBtnSpeaker.classList.add('active');}else{callBtnSpeaker.textContent='🔈';callBtnSpeaker.classList.remove('active');} }
...
function showCallScreen(name, avatar, status, isVideo) {
  ...
  if (callScreenAvatar) { if(avatar)callScreenAvatar.innerHTML=`<img src="${avatar}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;else callScreenAvatar.textContent='👤'; }
  ...
  callScreen.classList.add('active'); callScreen.classList.remove('minimizing');
  hideCallMiniBar();
  const bottom = callScreen.querySelector('.call-screen-bottom');
  const top    = callScreen.querySelector('.call-screen-top');
  ...
  callControlsVisible = true; clearTimeout(callControlsHideTimer);
  if (isVideo) { ensureVideoElements(); ensureLocalVideo(); showVideoUI(true); startLocalVideo().then(s=>{if(s&&callBtnVideo){callBtnVideo.classList.add('active');pcCallIsVideo=true;}callControlsHideTimer=setTimeout(hideCallControls,4000);}); }
}
...
function hideCallScreen() {
  ...
  callScreen.addEventListener('animationend', () => { callScreen.classList.remove('active'); callScreen.classList.remove('minimizing'); }, {once:true});
  stopCallTimer();
  if (pcCallActive) { const img=callScreenAvatar?.querySelector('img'); showCallMiniBar(pcCallRemoteNick, img?img.src:null); updateCallMiniStatus(callScreenStatus?.textContent||'Звонок…'); }
}
...
function ensureVideoElements() {
  ...
  if (!vc) {
    vc = document.createElement('div'); vc.id='call-video-container'; vc.style.cssText='position:fixed;inset:0;display:none;z-index:998;background:#000;overflow:hidden;';
    const rv=document.createElement('video'); rv.id='video-remote'; rv.autoplay=true; rv.playsInline=true; rv.style.cssText='width:100%;height:100%;object-fit:cover;'; vc.appendChild(rv);
    const ns=document.createElement('div'); ns.id='video-no-signal'; ns.style.cssText='position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;color:rgba(255,255,255,0.4);font-size:14px;gap:12px;'; ns.innerHTML='<span style="font-size:48px">📷</span><span>Видео недоступно</span>'; vc.appendChild(ns);
...
function ensureLocalVideo() {
  ...
  if (!lv) { lv=document.createElement('video'); lv.id='video-local'; lv.autoplay=true; lv.playsInline=true; lv.muted=true; document.body.appendChild(lv); }
  return lv;
}

async function startLocalVideo() {
  try {
    localVideoStream = await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:1280},height:{ideal:720}},audio:false});
    const lv = ensureLocalVideo(); lv.srcObject = localVideoStream; lv.style.display = 'block';
...
function stopLocalVideo() {
  ...
  const lv = document.getElementById('video-local'); if(lv){lv.srcObject=null;lv.style.display='none';}
}

function showVideoUI(show) {
  const vc=document.getElementById('call-video-container'); if(vc)vc.style.display=show?'block':'none';
  const lv=document.getElementById('video-local'); if(lv)lv.style.display=show?'block':'none';
}
...
function endPrivateCall(notify = true) {
  if (notify && (pcCallRemoteId || pcCallRemoteNickLow)) socket.emit('private-call-end', {to: pcCallRemoteId || pcCallRemoteNickLow});
  clearTimeout(callControlsHideTimer);
  ...
  pcCallRemoteNick=''; pcCallMuted=false; isSpeakerMode=false; pcIceCandidateBuffer=[];
  ...
  stopCallTimer();
  const center=callScreen?.querySelector('.call-screen-center'); if(center)center.style.display='flex';
  const bottom=callScreen?.querySelector('.call-screen-bottom'); const top=callScreen?.querySelector('.call-screen-top');
...
async function startPrivateCall(isVideo) {
  ...
  pcCallRemoteId      = null; pcIceCandidateBuffer = []; pcCallIsVideo = isVideo;
  try { pcCallStream = await navigator.mediaDevices.getUserMedia({audio:true,video:false}); }
  catch (e) { showToast('❌ Нет доступа к микрофону'); return; }
  ...
  if (isVideo) {
    ...
    if (vs && pcCallPeer) { const vt=vs.getVideoTracks()[0]; if(vt){try{pcCallPeer.addTrack(vt,vs);}catch(_){}} }
  }
  playDialTone();
...
function createPrivateCallPeer(targetId, isInitiator, isVideo) {
  const peer = new RTCPeerConnection(iceServers);
  if (isInitiator) { peer.addTransceiver('audio',{direction:'sendrecv'}); if(isVideo)peer.addTransceiver('video',{direction:'sendrecv'}); }
  peer.ontrack = e => {
    const track  = e.track;
    const stream = e.streams&&e.streams[0]?e.streams[0]:new MediaStream([track]);
    if (track.kind === 'audio') {
      let audio = document.getElementById('audio-pc-call');
      if (!audio) { audio=document.createElement('audio'); audio.id='audio-pc-call'; audio.autoplay=true; audio.playsInline=true; document.body.appendChild(audio); }
      audio.srcObject = stream; audio.volume = isSpeakerMode?1.0:0.7;
      audio.play().catch(()=>{ const resume=()=>{audio.play().catch(()=>{})}; document.addEventListener('click',resume,{once:true}); document.addEventListener('touchstart',resume,{once:true}); });
    }
    if (track.kind === 'video') {
      ensureVideoElements(); showVideoUI(true); pcCallIsVideo=true;
      if(callBtnVideo)callBtnVideo.classList.add('active');
      const vc=document.getElementById('call-video-container'); if(vc)vc.style.display='block';
      const rv=document.getElementById('video-remote'); const ns=document.getElementById('video-no-signal');
      if (rv) {
        let remoteStream=rv.srcObject; if(!remoteStream||(!(remoteStream instanceof MediaStream))){remoteStream=new MediaStream();rv.srcObject=remoteStream;}
        remoteStream.getVideoTracks().forEach(t=>remoteStream.removeTrack(t)); remoteStream.addTrack(track);
        rv.play().then(()=>{if(ns)ns.style.display='none';const center=callScreen?.querySelector('.call-screen-center');if(center)center.style.display='none';showCallControls();}).catch(err=>{const resume=()=>{rv.play().catch(()=>{})};document.addEventListener('click',resume,{once:true});document.addEventListener('touchstart',resume,{once:true});});
      }
    }
  };
  ...
  peer.onconnectionstatechange = () => {
    const state = peer.connectionState;
    if (state==='connected') { stopDialTone();pcCallActive=true;startCallTimer();showToast('🟢 Звонок установлен',2000);setSpeakerOutput(pcCallIsVideo?true:isSpeakerMode);setCallStatus('Соединён');if(pcCallIsVideo)showCallControls(); }
    if (state==='connecting')   setCallStatus('Соединение…');
    if (state==='disconnected') { setCallStatus('Переподключение…'); setTimeout(()=>{if(peer.connectionState==='disconnected'||peer.connectionState==='failed'){showToast('📵 Соединение прервано',3000);endPrivateCall(false);}},6000); }
    if (state==='failed')       { showToast('📵 Не удалось соединиться',3000); endPrivateCall(false); }
  };
  ...
  if (isInitiator) {
    ...
    peer.onnegotiationneeded = async () => {
      if (offerCreated) return; offerCreated = true;
      try {
        const offer = await peer.createOffer({offerToReceiveAudio:true,offerToReceiveVideo:isVideo});
        await peer.setLocalDescription(offer);
        socket.emit('private-call-offer', {chatId:currentChatId, to:pcCallRemoteNickLow, offer:peer.localDescription, isVideo});
      } catch (e) { console.error('Private call offer error:', e); offerCreated=false; showToast('❌ Ошибка при установке звонка',3000); endPrivateCall(false); }
    };
  }
  return peer;
}
...
// ═══════════════════════════════════════════════
//  SOCKET EVENTS — ЗВОНКИ
// ═══════════════════════════════════════════════
socket.on('private-call-offer', async data => {
  if (pcCallActive) { socket.emit('private-call-reject',{to:data.from}); return; }
  incomingCallData = data;
  if (incomingCallAvatar) { if(data.fromAvatar)incomingCallAvatar.innerHTML=`<img src="${data.fromAvatar}" style="width:100%;height:100%;border-radius:50%;object-fit:cover">`;else incomingCallAvatar.textContent='👤'; }
  if (incomingCallName) incomingCallName.textContent = data.fromNick || '?';
  const subEl = $('incoming-call-sub'); if(subEl)subEl.textContent=data.isVideo?'📹 Видеозвонок…':'📞 Голосовой вызов…';
...
socket.on('private-call-answer', async ({from, answer}) => {
  ...
  pcCallRemoteId = from; stopDialTone();
  if (pcCallPeer.signalingState !== 'have-local-offer') { console.warn('Unexpected signalingState on answer:', pcCallPeer.signalingState); return; }
  try { await pcCallPeer.setRemoteDescription(new RTCSessionDescription(answer)); }
  catch (e) { console.error('setRemoteDescription (answer) error:', e); showToast('❌ Ошибка установки соединения',3000); endPrivateCall(false); return; }
  for (const candidate of pcIceCandidateBuffer) socket.emit('private-call-ice',{to:pcCallRemoteId,candidate});
  pcIceCandidateBuffer = [];
});

socket.on('private-call-ice', async ({from, candidate}) => {
  if (!candidate) return;
  if (pcCallPeer && pcCallPeer.remoteDescription) { try{await pcCallPeer.addIceCandidate(new RTCIceCandidate(candidate));}catch(_){} }
...
socket.on('existing-voice-users', async users => {
  for (const user of users) {
    ...
    try { if(!ecdhExchanged.has(user.id)){ecdhExchanged.add(user.id);socket.emit('ecdh-pubkey',{to:user.id,pubkey:await Crypto.exportPublicKey()});} } catch(_) {}
  }
});
...
socket.on('voice-user-joined', async data => {
  ...
  if (joined) {
    ...
    try{if(!ecdhExchanged.has(uid)){ecdhExchanged.add(uid);socket.emit('ecdh-pubkey',{to:uid,pubkey:await Crypto.exportPublicKey()});}}catch(_){}
  }
});

socket.on('offer',          async ({from,offer,nickname}) => { if(nickname)voiceNicknames[from]=nickname; if(!localStream){pendingOffers.push({from,offer,nickname});return;} await handleOffer(from,offer,nickname); });
socket.on('answer',         async ({from,answer}) => { if(voiceNicknames[from])updateParticipantName(from,voiceNicknames[from]); const peer=peers[from]; if(peer&&peer.signalingState==='have-local-offer'){try{await peer.setRemoteDescription(new RTCSessionDescription(answer));}catch(e){console.error('answer setRemoteDescription error:',e);}} });
socket.on('ice-candidate',  async ({from,candidate}) => { const peer=peers[from]; if(peer&&candidate){try{await peer.addIceCandidate(new RTCIceCandidate(candidate));}catch(_){}} });
socket.on('voice-user-left', uid => { playBeep('leave');removeParticipant(uid);stopVolumeAnalysis(uid);stopQualityMonitor(uid);delete voiceNicknames[uid];if(peers[uid]){peers[uid].close();delete peers[uid];}const el=document.getElementById('audio-'+uid);if(el)el.remove(); });
socket.on('understood', ({from,nickname}) => { playOkSound(); const b=document.createElement('div');b.className='understood-banner';b.textContent='✅ Понял! ('+(nickname||shortId(from))+')';document.body.appendChild(b);setTimeout(()=>b.remove(),3000); });
...
document.addEventListener('visibilitychange', async () => {
  ...
  if (tracks.every(t => t.readyState === 'ended')) {
    try {
      ...
      for (const uid in peers) { const sender=peers[uid].getSenders().find(s=>s.track?.kind==='audio'); if(sender&&procTrack)await sender.replaceTrack(procTrack); }
      const newTrack = newRaw.getAudioTracks()[0];
      tracks.forEach(t=>{localStream.removeTrack(t);t.stop();}); localStream.addTrack(newTrack);
      processedStream=newProc; stopVolumeAnalysis(socket.id); startVolumeAnalysis(socket.id,localStream); newTrack.enabled=micEnabled;
    } catch(_) {}
...
(function injectStyles() {
  ...
  style.textContent = `
    ...
    .group-photo-change-btn { display:flex;align-items:center;gap:10px;padding:12px 16px;border-radius:14px;border:1px solid rgba(124,92,191,0.2);background:var(--bg2);color:var(--accent2);font-size:14px;cursor:pointer;width:100%;margin-bottom:12px; }
    ...
    #call-screen .call-screen-top { position:fixed!important;top:env(safe-area-inset-top,0)!important;left:0;right:0;z-index:1001!important;padding:max(env(safe-area-inset-top),12px) 20px 12px!important;background:linear-gradient(to bottom,rgba(0,0,0,0.6),transparent)!important; }
    #call-screen .call-screen-center { position:fixed!important;top:50%!important;left:0;right:0;transform:translateY(-50%)!important;z-index:1001!important;background:none!important;padding:20px!important; }
    #call-screen .call-screen-bottom { position:fixed!important;bottom:0!important;left:0;right:0;z-index:1001!important;padding:20px 24px max(env(safe-area-inset-bottom),24px)!important;background:linear-gradient(to top,rgba(0,0,0,0.75),transparent)!important;border-radius:0!important; }
    #video-local { position:fixed!important;top:max(calc(env(safe-area-inset-top,0px) + 60px),80px)!important;right:16px!important;width:90px!important;height:130px!important;border-radius:14px!important;object-fit:cover!important;border:2px solid rgba(255,255,255,0.4)!important;box-shadow:0 4px 16px rgba(0,0,0,0.6)!important;z-index:1002!important;cursor:pointer!important;background:#222!important; }
  `;
...
document.addEventListener('DOMContentLoaded', () => {
  // 1. Инициализируем DOM-переменные
  initDOM();

  // 2. Применяем тему
  if (typeof applyTheme === 'function') applyTheme(currentTheme);

  // 3. Инициализируем UI (нижняя навигация, поиск и т.д.)
  if (typeof initUI === 'function') initUI();

  // 4. Вешаем обработчики событий
  initEventListeners();

  // 5. Попытка авто-логина
  tryAutoLogin();
});
...
function initEventListeners() {
  ...
  $('room-photo-input')?.addEventListener('change', () => {
    ...
    const file = rpi?.files[0]; if (!file) return;
    if (rpi) rpi.value = '';
  ...
  $('btn-toggle-create-pw')?.addEventListener('click', () => {
    const i = $('create-room-pw'); if (!i) return;
    const t = i.type === 'text'; i.type = t ? 'password' : 'text';
    const btn = $('btn-toggle-create-pw'); if (btn) btn.textContent = t ? '👁' : '🙈';
  });
  ...
  $('btn-toggle-room-pw')?.addEventListener('click', () => {
    const i = $('room-pw-input'); if (!i) return;
    const t = i.type === 'text'; i.type = t ? 'password' : 'text';
    const btn = $('btn-toggle-room-pw'); if (btn) btn.textContent = t ? '👁' : '🙈';
  });
  ...
  $('btn-rename-room')?.addEventListener('click', () => {
    ...
    socket.emit('room-rename', {roomId:currentRoomId, newName:name}, res => {
      ...
      else { const re=$('rename-error'); if(re) re.textContent = res.error==='not_owner'?'❌ Нет прав':'⚠️ Ошибка'; }
    });
  });
  ...
  $('group-photo-input')?.addEventListener('change', () => {
    ...
    const file = gpi?.files[0]; if (!file) return;
    if (gpi) gpi.value = '';
    if (file.size > 5*1024*1024) { showToast('⚠️ Фото слишком большое'); return; }
    const r = new FileReader();
    r.onload = e => {
  ...
  // ─── Chat header ───
  $('btn-back-lobby')?.addEventListener('click', () => { leaveCurrentRoom(); showScreen('lobby'); });
  ...
  $('btn-join')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const bj=$('btn-join');  if(bj)  bj.style.display  = 'none';
      const bl=$('btn-leave'); if(bl)  bl.style.display  = 'block';
      const bm=$('btn-mic');   if(bm)  bm.style.display  = 'block';
      joined = true;
  ...
  $('btn-leave')?.addEventListener('click', () => {
    ...
    const bj=$('btn-join');  if(bj)  bj.style.display  = 'block';
    const bl=$('btn-leave'); if(bl)  bl.style.display  = 'none';
    const bm=$('btn-mic');   if(bm)  bm.style.display  = 'none';
    const ms=$('mic-status');if(ms){ms.className='mic-status';ms.textContent='';}
  ...
  $('btn-mic')?.addEventListener('click', () => {
    ...
    const bm=$('btn-mic');
    if(bm) bm.textContent = micEnabled ? '🔇 Выключить микрофон' : '🎙️ Включить микрофон';
  });

  // ─── Chat input ───
  $('chat-input')?.addEventListener('input', () => {
    const ci = $('chat-input'); if (!ci) return;
    ci.style.height = 'auto';
    ci.style.height = Math.min(ci.scrollHeight, 120) + 'px';
    const bvr = $('btn-voice-record');
    if (bvr) bvr.style.display = ci.value.trim().length > 0 ? 'none' : 'flex';
    if (ci.value.trim().length > 0) {
      if (currentChatType === 'private') startPrivateTyping(); else startMyTyping();
    } else {
      if (currentChatType === 'private') stopPrivateTyping(); else stopMyTyping();
    }
  });
  ...
  $('btn-photo')?.addEventListener('click', () => {
    const fi = $('file-input'); if(fi){ fi.accept='image/*'; fi.click(); }
  });
  $('btn-video')?.addEventListener('click', () => {
    const fi = $('file-input'); if(fi){ fi.accept='video/*'; fi.click(); }
  });
  $('btn-file')?.addEventListener('click', () => {
    const fi = $('file-input'); if(fi){ fi.accept='*/*'; fi.click(); }
  });
  ...
  $('lightbox-close')?.addEventListener('click', () => {
    const lb=$('lightbox');const lc=$('lightbox-content');
    if(lb)lb.classList.remove('open');if(lc)lc.innerHTML='';
  });
  $('lightbox')?.addEventListener('click', e => {
    if (e.target === $('lightbox')) {
      $('lightbox')?.classList.remove('open');
      const lc=$('lightbox-content');if(lc)lc.innerHTML='';
    }
  });
  ...
  if (bvr) {
    ...
    let touchStartX = 0;
    bvr.addEventListener('touchmove', e => {
      if (!isVoiceRecording) return;
  ...
  $('call-btn-mute')?.addEventListener('click', () => {
    ...
    if (pcCallMuted) { btn?.classList.add('active');    if(btn)btn.textContent='🔇'; }
    else             { btn?.classList.remove('active'); if(btn)btn.textContent='🎤'; }
  ...
  $('call-btn-video')?.addEventListener('click', async () => {
    ...
    if (!pcCallIsVideo) {
      ...
      pcCallIsVideo = true; btn?.classList.add('active'); showVideoUI(true);
      if (pcCallPeer) {
  ...
  $('call-screen')?.addEventListener('click', e => {
    ...
    if (pcCallIsVideo) {
      if (callControlsVisible) hideCallControls(); else showCallControls();
    }
  });
  ...
  $('btn-call-accept')?.addEventListener('click', async () => {
    ...
    pcCallStream.getAudioTracks().forEach(t => { try{pcCallPeer.addTrack(t,pcCallStream);}catch(_){} });
    try { await pcCallPeer.setRemoteDescription(new RTCSessionDescription(data.offer)); }
    catch (e) { console.error('setRemoteDescription error:', e); showToast('❌ Ошибка установки соединения'); endPrivateCall(false); return; }
    ...
    } catch (e) { console.error('createAnswer error:', e); showToast('❌ Ошибка ответа на звонок'); endPrivateCall(false); return; }
  ...
  document.addEventListener('click', e => {
    if (e.target && e.target.id === 'video-local' && localVideoStream) {
      ...
      navigator.mediaDevices.getUserMedia({
        ...
        ct.stop();
        const lv = $('video-local'); if(lv) lv.srcObject = ns;
        localVideoStream = ns;
      }).catch(() => showToast('❌ Ошибка переключения камеры'));
    }
  });

  // ─── Long press для сообщений ───
  initLongPress();
  ...
  // ─── Drawer avatar click → profile ───
  $('drawer-avatar')?.addEventListener('click', () => {
    if (typeof closeDrawer === 'function') closeDrawer();
    openProfileModal();
  });

  // ─── Visibility change — unread ───
  ...
  (function initUsernameCheck() {
    ...
    rn?.addEventListener('input', () => {
      ...
      if (suggestion.length >= 2) { input.value = suggestion; checkUsername(suggestion); }
    });

    let timer = null;
    function checkUsername(val) {
      val = val.trim().toLowerCase().replace(/[^a-z0-9_]/g,'');
      if (!val || val.length < 2) { status.className='username-status'; status.textContent=''; return; }
      status.className='username-status loading'; status.textContent='⏳ Проверяем…';
      clearTimeout(timer);
      timer = setTimeout(() => {
...
// ═══════════════════════════════════════════════
socket.on('connect', () => {
  const rb = $('reconnect-banner'); if(rb) rb.classList.remove('visible');
  if (myNickname) {
    socket.emit('set-nickname', myNickname, () => {
      if (currentRoomId && currentChatType === 'group')   joinRoom(currentRoomId, currentPassword);
      if (currentChatId && currentChatType === 'private') socket.emit('private-chat-join', {chatId:currentChatId});
...
socket.on('disconnect', () => {
  if ((currentRoomId || currentChatId) && $('reconnect-banner')) {
