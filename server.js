...
app.put('/api/me', async (req, res) => {
  ...
  try {
    await pool.query(
      ...
      [avatar ?? user.avatar, cleanBio, user.id]
    );
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ ok: false, error: 'server_error' });
...
app.post('/api/rooms', async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [roomRes.rows[0].id, user.id, 'owner']
    );
...
app.get('/api/rooms/:id/messages', async (req, res) => {
  try {
    ...
    if (room.is_private || room.pass_hash) {
      const member = await pool.query('SELECT 1 FROM room_members WHERE room_id = $1 AND user_id = $2', [room.id, user.id]);
